...
class FedGR(torch.nn.Module):
    ...
    def __init__(self, num_tasks, num_layer = 5, emb_dim = 300, gnn_type = 'gin', drop_ratio = 0.5, gamma = 0.4, use_linear_predictor=False):
        ...
        self.node_enoder = nn.Linear(4,emb_dim)


    def forward(self, x, edge_index, edge_attr, batch):

        x_feature = self.node_enoder(x.float())
        h_node = self.graph_encoder(x_feature,edge_index, edge_attr.long(), batch)
        ##  rationale
        h_r, h_env, r_node_num, env_node_num = self.separator(x_feature,edge_index, edge_attr.long(), batch, h_node)
    ...
    def get_contrastive_loss(self,x,x_aug,x_cp,T=1):
        batch_size, _ = x.size()
        x_abs = x.norm(dim=1)
    ...
    def eval_forward(self, x, edge_index, edge_attr, batch):
        ...
        h_r, _, _, _ = self.separator(x_feature,edge_index, edge_attr.long(), batch, h_node)
